...
sub compress {
    my $source = shift;
    return wd_qlz_compress($source, length($source));
}

sub decompress {
    my $source = shift;
    return wd_qlz_decompress($source);
}
...
SV * wd_qlz_compress(char * source, size_t size) {
    char * wbuf = (char*) malloc(QLZ_SCRATCH_COMPRESS);
    char * dst = (char *) malloc(size + 400);
    size_t cp_size = qlz_compress(source, dst, size, wbuf);
    SV * sv = newSVpvn(dst, cp_size);
    free(wbuf);
    free(dst);
    return sv;
}

SV * wd_qlz_decompress(char * source) {
    int size = qlz_size_decompressed(source);
    char * wbuf = (char*) malloc(QLZ_SCRATCH_DECOMPRESS);
    char * dst = (char *) malloc(size);

    size_t decomp_size = qlz_decompress(source, dst, wbuf);
    SV * sv = newSVpvn(dst, decomp_size);
    free(wbuf);
    free(dst);
    return sv;
}
...
//
// --- Below code and quicklz.h is from beansdb
//
